...
def download_file(url, directory):
    ...
    # Get the filename from URL and normalize it
    original_filename = url.split('/')[-1]
    clean_filename = normalize_filename(original_filename)
    local_filename = os.path.join(directory, clean_filename)
...
def get_zip_links(url, directory):
        ...
        for link in soup.find_all('a'):
            if link and 'href' in link.attrs:
                ...
                if href.endswith(('.wsc', '.tic', '.sg', '.zip','.mgw', '.gbc', '.lnx','.gba', '.rar', 
                    ...
                    print(f"Starting download: {full_url}")
                    download_file(full_url, directory)
    except Exception as e:
