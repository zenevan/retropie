...
def get_all():
    ...
    # Create batch file to run downloads
    with open('run_downloads.cmd', 'w', encoding='utf-8') as f:
        f.write("@echo off\n")
        f.write("echo Creating directories...\n")
        # Create all directories first
        for dir in dirs.values():
            f.write(f"mkdir \"{dir}\" 2>nul\n")
        f.write("\necho Starting downloads...\n")
        f.write("for /F \"usebackq tokens=*\" %%A in (\"downloads.txt\") do (\n")
        f.write("    echo Processing: %%A\n")
        f.write("    %%A\n")
        f.write(")\n")
